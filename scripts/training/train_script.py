...
def main():
    ...
    parser.add_argument('--base', dest='base_model', type=str, help='Base model (huggingface name or path to GPTQ folder) to finetune')
    ...
    parser.add_argument('--bits', dest='bits', type=int, help='Number of bits of quantised model')

    parser.add_argument('--output-path', dest='output', type=str, help='Output path', default = str(pathlib.Path(__file__).parent.resolve()) + "/../../backend/models")
    ...
    parser.add_argument('--has-system-prompt', dest='has_sys', type=int, help='Does the model support system prompts in their chat template 1 or 0', default=1)
    parser.add_argument('--max-len', dest='max_len', type=int, help='Max generation length', default=256)
    parser.add_argument('--seed', dest='seed', type=int, help='Seed used when splitting dataset. The value -1 will be interpreted as an un-set seed', default=-1)
    parser.add_argument('--group_size', type=int, default=128, help='Specify GPTQ group_size.')
    # Parse the arguments
