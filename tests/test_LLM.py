...
from backend.src.llm_classes.llm_level import GPT, LLM, ChatLog
...
class TestChatLogAndGPT(TestCase):
    ...
    @patch(
        "backend.src.llm_classes.llm_level.OpenAI"
    )  # Adjust the module path as necessary
    ...
    def test_chatlog_multiple_messages(self):
        ...
        self.assertEqual(
            len(chat_log.message_list), 4
        )  # Includes initial system message
