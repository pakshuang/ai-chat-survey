...
def login_admin():
    ...
    if data['username'] not in admins or not check_password_hash(admins[data['username']], data['password']):
...
def get_surveys():
    ...
    if username:
        surveys = {"surveys": [survey for survey in surveys.surveys if survey.metadata.created_by == username]} # Mock
...
def get_survey(survey_id):
    ...
    surveys = [survey for survey in surveys.surveys if survey.metadata.id == survey_id] # Mock
    if not surveys:
...
def get_responses(survey_id):
    ...
    responses = {"responses": [response for response in responses.responses if response.metadata.survey_id == survey_id]}
...
def get_response(survey_id, response_id):
    ...
    responses = [response for response in responses.responses if response.metadata.response_id == response_id]
    if not responses:
...
def send_chat_message(survey_id, response_id):
    ...
    return jsonify({'content': 'This is a dummy reply from the chatbot.', 'is_last': is_last}), 201
